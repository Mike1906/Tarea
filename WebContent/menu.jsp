<link rel="stylesheet" href="foundation/foundation.css">

<nav class="top-bar" data-topbar role="navigation">
	<ul class="title-area">
		<li class="name">
			<h1>
				<a href="#">Tarea de Ingenieria de Software</a>
			</h1>
		</li>
	</ul>
	<section class="top-bar-section">
		<!-- Right Nav Section -->
		<ul class="right">
			<li><a href="Main.jsp">Inicio</a></li>
			<li class="divider hide-for-small"></li>
			<li class="has-dropdown"><a href="#">Insertar Datos</a>
				<ul class="dropdown">
					<li><a href="IngresarPartidoPolitico.html">Partido Politico</a></li>
					<li><a href="CrearDepartamento.html">Departamento</a></li>
					<li><a href="CrearMunicipio.html">Municipio</a></li>
					<li><a href="IngresarJrv.html">JRV</a></li>
					<li><a href="CrearMiembrojrv.html">Miembro JRV</a></li>
					<li><a href="CrearTipoMiembro.html">Tipo Miembro</a></li>
					<li><a href="IngresarPadron.html">Padron Electoral</a></li>
					<li><a href="IngresarVotacion.html">Votacion</a></li>
					<li><a href="CrearUrna.html">Urna</a></li>
				</ul></li>
			<li class="divider hide-for-small"></li>
			<li class="has-dropdown"><a href="#">Consultar Datos</a>
				<ul class="dropdown">
					<li><a href="ConsultarPartidoPolitico.jsp">Partido Politico</a></li>
					<li><a href="ConsultarDepartamento.jsp">Departamento</a></li>
					<li><a href="ConsultarMunicipio.jsp">Municipio</a></li>
					<li><a href="ConsultarJrv.jsp">JRV</a></li>
					<li><a href="ConsultarMiembrojrv.jsp">Miembro JRV</a></li>
					<li><a href="ConsultarTipoMiembro.jsp">Tipo Miembro</a></li>
					<li><a href="ConsultarPadron.jsp">Padron Electoral</a></li>
					<li><a href="ConsultarVotacion.jsp">Votacion</a></li>
					<li><a href="#">Urna</a></li>
				</ul></li>
			<li class="divider hide-for-small"></li>
			<li class="has-dropdown"><a href="#">Actualizar Datos</a>
				<ul class="dropdown">
					<li><a href="ActualizarPartidoPolitico.html">Partido Politico</a></li>
					<li><a href="ActualizarDepartamento.html">Departamento</a></li>
					<li><a href="ActualizarMunicipio.html">Municipio</a></li>
					<li><a href="ActualizarJrv.html">JRV</a></li>
					<li><a href="ActualizarMiembrojrv.html">Miembro JRV</a></li>
					<li><a href="ActualizarTipoMiembro.html">Tipo Miembro</a></li>
					<li><a href="ActualizarPadron.html">Padron Electoral</a></li>
					<li><a href="ActualizarVotacion.html">Votacion</a></li>
					<li><a href="#">Urna</a></li>
				</ul></li>
			<li class="divider hide-for-small"></li>
			<li class="has-dropdown"><a href="#">Eliminar Datos</a>
				<ul class="dropdown">
					<li><a href="EliminarPartidoPolitico.html">Partido Politico</a></li>
					<li><a href="BorrarDepartamento.html">Departamento</a></li>
					<li><a href="BorrarMunicipio.html">Municipio</a></li>
					<li><a href="EliminarJrv.html">JRV</a></li>
					<li><a href="BorrarMiembrojrv.html">Miembro JRV</a></li>
					<li><a href="BorrarTipoMiembro.html">Tipo Miembro</a></li>
					<li><a href="EliminarPadron.html">Padron Electoral</a></li>
					<li><a href="EliminarVotacion.html">Votacion</a></li>
					<li><a href="#">Urna</a></li>
				</ul></li>
			<li class="divider hide-for-small"></li>
			<li class="has-dropdown"><a href="#">Usuario</a>
				<ul class="dropdown">
					<li><a href="CrearUsuario.html">Crear Usuario</a></li>
					<li><a href="ConsultarUsuario.jsp">Consultar Usurios</a></li>
					<li><a href="BorrarUsuario.html">Borrar Usuario</a></li> 
				</ul></li>
			<li class="divider hide-for-small"></li>
			<li><a href="login.jsp">Cerrar Sesion</a></li>
		</ul>
	</section>
</nav>
